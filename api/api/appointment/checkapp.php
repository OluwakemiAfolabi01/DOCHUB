...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($appointment->validate_params($_POST['mode'])){
        ...
        echo json_encode(array('success' => 0, "message" => 'Appointment mode is required!'));
        die();
    }
    
    if($appointment->check_appointment()){
        if($appointment->check_valid_mode()){
            echo json_encode(array('success' => 1, "message" => 'Appointment Found'));
        }else{
            echo json_encode(array('success' => 2, "message" => 'Invalid Appointment Mode'));
        }
    }else{
